...
let rec level' t i =
  match t,i with
  | Empty, _ -> []
  | Node(d,_,_), 1 -> [d] (* '_' because we aren't using the lt and rt *)
  | Node(_,lt,rt), n -> level lt (i-1) @ level rt (i-1) (* ditto for this line *)
...
(* this solution pretty much matches Bonelli's - prob cuz he helped us LOL *)
let rec height t= 
  match t with
  | Empty -> 0
...
*)
(* Audie being a damn legend by one-shotting this during the quiz *)
let rec pbt h d =
   match h with
   | 0 -> Empty
